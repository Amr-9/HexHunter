/* vanity.cl - Fixed & Optimized for Ethereum Vanity Address */
...
__constant const ulong keccakf_rndc[24] = {
...
__constant const int keccakf_rotc[24] = {
    1,  3,  6,  10, 15, 21, 28, 36, 45, 55, 2,  14,
    27, 41, 56, 8,  25, 43, 62, 18, 39, 61, 20, 44
};

__constant const int keccakf_piln[24] = {
...
void sha3_keccakf(ethhash * const h) {
    ...
    // NOTE: Padding must be applied BEFORE calling this function!
    // For 64-byte input: h.d[16] ^= 0x01; h.b[135] ^= 0x80;
    ...
    for (int round = 0; round < 24; ++round) {
        ...
        for (int i = 0; i < 5; ++i) {
            ulong t = bc[(i + 4) % 5] ^ rotate(bc[(i + 1) % 5], (ulong)1);
            st[i     ] ^= t;
            st[i +  5] ^= t;
            st[i + 10] ^= t;
            st[i + 15] ^= t;
...
mp_word mp_sub_mod(mp_number * const r) {
    mp_number m = { {0xfffffc2f, 0xfffffffe, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff} };
    mp_word t, c = 0;
    for (mp_word i = 0; i < MP_WORDS; ++i) {
        t = r->d[i] - m.d[i] - c;
...
__kernel void profanity_init(
    ...
    // Set p to the computed random point directly
    p = p_random;

    mp_number gx = { {0x16f81798, 0x59f2815b, 0x2dce28d9, 0x029bfcdb, 0xce870b07, 0x55a06295, 0xf9dcbbac, 0x79be667e} };
    mp_mod_sub(&tmp1, &p.x, &gx);
    mp_mod_inverse(&tmp1);

    mp_number gy = { {0xfb10d4b8, 0x9c47d08f, 0xa6855419, 0xfd17b448, 0x0e1108a8, 0x5da4fbfc, 0x26a3c465, 0x483ada77 } };
    mp_mod_sub(&tmp2, &p.y, &gy);
    mp_mod_mul(&tmp1, &tmp1, &tmp2);

    mp_mod_sub(&p.x, &p.x, &gx);
...
__kernel void profanity_inverse(
    __global const mp_number * const pDeltaX,
...
__kernel void profanity_iterate(
    ...
    // Apply Keccak padding for 64-byte input
    // Start padding: byte 64 (after the 64 bytes of public key)
    h.d[16] ^= 0x01;
    // End padding: byte 135 (rate - 1 for Keccak-256, rate = 136)
    h.b[135] ^= 0x80;

    sha3_keccakf(&h);

    // Store the last 20 bytes of Keccak hash (Ethereum address)
    // h.d[3..7] = bytes 12-31 of the 256-bit hash
    // When read as bytes on little-endian, this gives correct order
...
// FIXED: Condition changed to >= to fix off-by-one error
// FIXED: Removed __global from hash pointer to support local arrays
// FIXED: Condition changed to > to strictly match score requirements
...
__kernel void profanity_score_matching(
    ...
    // Read hash bytes directly from memory like profanity2
    // The hash is stored in pInverse[id].d as raw bytes (20 bytes = 5 uint32s)
    __global const uchar * const hash = (__global const uchar *)pInverse[id].d;
...
__kernel void profanity_score_leading(
    __global mp_number * const pInverse,
    __global result * const pResult,
    __constant const uchar * const data1,
    __constant const uchar * const data2,
    ...
    int score = 0;
    for (int i = 0; i < 20; ++i) {
        if ((hash[i] & 0xF0) >> 4 == data1[0]) { ++score; } else { break; }
        if ((hash[i] & 0x0F) == data1[0]) { ++score; } else { break; }
    }
    profanity_result_update(id, hash, pResult, score, scoreMax);
}
